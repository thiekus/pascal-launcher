...
{$ENDIF}
  ...
  SException = 'Class "%s" in module "%s" at %p:'#10'%s%s';
...
{ Exceptions }
  ...
  protected
    FMessage: KOLString;  //dufa. moved to first position like in SysUtils.Exception class
    FCode: TError;
    FErrorCode: DWORD;
...
function ExceptionErrorMessage(ExceptObject: TObject; ExceptAddr: Pointer; Buffer: PKOLChar; Size: Integer): Integer;
var
  S:                KOLString;
  ex:               Exception;
  MsgPtr:           PKOLChar;
  ModuleName:       array[0..MAX_PATH] of KOLChar;
  Fmt:              array[Byte] of KOLChar;
  Info:             TMemoryBasicInformation;
  ConvertedAddress: Pointer;
begin
  VirtualQuery(ExceptAddr, Info, sizeof(Info));
  if (Info.State <> MEM_COMMIT) or
  (GetModuleFilename(THandle(Info.AllocationBase), ModuleName, SizeOf(ModuleName)) = 0) then begin
    GetModuleFileName(HInstance, ModuleName, SizeOf(ModuleName));
    ConvertedAddress := ConvertAddr(ExceptAddr);
  end else                  
    ConvertedAddress := Pointer(Integer(ExceptAddr) - Integer(Info.AllocationBase));
  ...
  {$IFNDEF USE_RESOURCESTRING}
    {$IFDEF UNICODE_CTRLS}
      WStrCopy(Fmt, SException);
    {$ELSE}
      StrCopy(Fmt, SException);
    {$ENDIF}
  {$ELSE}
    LoadString(FindResourceHInstance(HInstance), PResStringRec(@SException).Identifier, Fmt, SizeOf(Fmt));
  {$ENDIF}
  S := Format(Fmt, [String(ExceptObject.ClassName), ModuleName, ConvertedAddress, MsgPtr, ''{MsgEnd}]);
  {$IFDEF UNICODE_CTRLS}
    WStrCopy(Buffer, PKOLChar(S));
  {$ELSE}
    StrCopy(Buffer, PKOLChar(S));
  {$ENDIF}
...
begin
  ...
  //begin
    ...
    MessageBox(0, Buffer, 'Raised exception', MB_ICONERROR or MB_SYSTEMMODAL);
  //end;
end;
